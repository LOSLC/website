...
function Comment({ comment, post }: { comment: CommentType; post: PostType }) {
  ...
  const isAuthor =
    usePage<SharedData>().props.auth.user?.id === comment.author.id;
      ...
      intervalRef.current = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) {
            ...
            router.delete(
              route("blog.comment.delete", { comment: comment.id }),
              {
                preserveScroll: true,
                onFinish: () => {
                  setIsDeleting(false);
          ...
          <img
            src={
              comment.author.avatar_url
                ? `/storage/${comment.author.avatar_url}`
            ...
            <CollapsibleTrigger asChild>
              {comment.replies && comment.replies.length > 0 ? (
                <Button
                  variant={"ghost"}
                  size="sm"
                  className="-ml-2 cursor-pointer gap-1 rounded-full px-2"
                >
